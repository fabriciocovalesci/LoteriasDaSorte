...
const GeradorLotoFacil = (navigation, route) => {
    ...
    async function savedData() {
        try {
            FavoritosDataBase.create({ titulo: text, numeros: JSON.stringify(loteriaMega.numeros), associar: checked, concurso: parseInt(megasena.proxConcurso), loteria: 'lotofacil', dataProxConcurso: megasena.dataProxConcurso })
                .then(id => {
    ...
    function returnMSG() {
        let message = `Palpite Loto Fácil para o concurso ${megasena.proxConcurso}:`
        loteriaMega.numeros.filter(elem => message = message + ' ' + elem)
        return message;
    }

    function setGenerate() {
        setloteriaMega({ nome: 'Loto Fácil', numeros: generate(1, 25, 15, sliderMegaPar, sliderMegaImpar) })
    }
    ...
    React.useEffect(() => {
        function initial() {
            setloteriaMega({ nome: 'Mega Sena', numeros: generate(1, 25, 15, null, null) })
                                                ...
                                                <View style={styles.cardShadow}>
                                                    ...
                                                    <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 10, justifyContent: "center" }}>
                                                        {elem.dezenas.length !== 0 ? elem.dezenas.map((dezena, index) =>
                                                            <View key={index} style={loteriaMega.numeros.toString().replace(/\s*\,\s*/g, ",").trim().split(",").includes(dezena) ? styles.circleFacil : styles.circle}>
                                                                <Text style={styles.fontText}>{dezena}</Text>
                                                            </View>
                                                        ) : null}
                                                    </View>
