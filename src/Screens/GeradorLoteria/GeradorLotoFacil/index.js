...
const GeradorLotoFacil = ({ route }) => {

    const [selected, setStateBtn] = React.useState(0)
    const [loteriaLotoFacil, setloteriaLotoFacil] = React.useState({ nome: '', numeros: '' });
    ...
    async function savedData() {
        try {
            FavoritosDataBase.create({ titulo: text, numeros: JSON.stringify(loteriaLotoFacil.numeros), associar: checked, concurso: parseInt(lotofacil.proxConcurso), loteria: 'lotofacil', dataProxConcurso: lotofacil.dataProxConcurso })
                .then(id => {
    ...
    function returnMSG(){
        let message = `Palpite Loto Fácil para o concurso ${lotofacil.proxConcurso}:`
        loteriaLotoFacil.numeros.filter(elem => message = message + ' ' + elem)
        return message;
    }
    ...
    function setGenerate(){
        setloteriaLotoFacil({ nome: 'Loto Fácil', numeros: generate(1, 25, 15) })
    }
    ...
    async function getFilters(){
        try {
            const filters = await FiltroDb.findByloteria('lotofacil');
            setFilterDB(filters)
    ...
    async function returnIdChildrenAplicarFiltro(id){
        ...
        setSelectedDB(data)
        setloteriaLotoFacil({ nome: 'Loto Fácil', numeros: await returnDataFiltro(data[0].maiorocorrencia, data[0].menorocorrencia, data[0].maioratraso, data[0].menoratraso, data[0].qtadepar, data[0].qtadeimpar, data[0].qtadedezenas, EstatisFacil) })
    ...
    React.useEffect(() => {
        async function loadDataMega() {
            const data = await ResultadoLotoFacil();
            setLotoFacil(data.data)
        }
        loadDataMega();
    }, []);
            ...
            <View>
              <Portal>
                <Dialog visible={props.visible} onDismiss={props.hideDialog}>
                  <Dialog.Title style={{ textAlign: "center", color: Colors.purple600 }}>Filtros Loto Fácil</Dialog.Title>
                  <Dialog.Content>
                    <ScrollView>
        ...
        <> 
            <View style={{ flexDirection: "row", justifyContent: "flex-end", alignItems: 'center', marginBottom: 10 }}>
                <View style={{ justifyContent: "center", alignItems: "flex-end", marginTop: 25 }}>
                    <Title style={{ textAlign: "center", fontSize: 18 }}>Surpresinha Loto Fácil</Title>
                </View>
            ...
            <View style={{ justifyContent: "center", alignContent: "space-around", flexDirection: "row" }}>
                {SelectedDB.length !== 0 ?
                    <Chip icon="close" onPress={removeFilter} style={{ alignItems: "baseline", backgroundColor: '#930989', height: 40 }}><Text style={{ color: "#fff", textAlign: "center", textAlignVertical: "center" }}>{SelectedDB[0].nome}</Text></Chip>
                : null}
            </View>

                <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 10, justifyContent: "center" }}>
                    {loteriaLotoFacil.numeros !== undefined && loteriaLotoFacil.numeros.length !== 0 ? loteriaLotoFacil.numeros.map((dezena, index) =>
            ...
            <Snackbar
                ...
                Números da {loteriaLotoFacil.title} salvo nos Favoritos.
            </Snackbar>
            </ScrollView>
        </>
    );
