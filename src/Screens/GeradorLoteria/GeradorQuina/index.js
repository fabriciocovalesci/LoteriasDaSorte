...
const GeradorQuina = ({ route }) => {
    ...
    async function savedData() {
        try {
            FavoritosDataBase.create({ titulo: text, numeros: JSON.stringify(loteriaQuina.numeros), associar: checked, concurso: parseInt(quina.proxConcurso), loteria: 'quina', dataProxConcurso: quina.dataProxConcurso })
                .then(id => {
    ...
    function returnMSG(){
        let message = `Palpite Quina para o concurso ${quina.proxConcurso}:`
        loteriaQuina.numeros.filter(elem => message = message + ' ' + elem)
        return message;
    }
    ...
    async function returnIdChildrenAplicarFiltro(id){
        ...
        setSelectedDB(data)
        setloteriaQuina({ nome: 'Quina', numeros: await returnDataFiltro(data[0].maiorocorrencia, data[0].menorocorrencia, data[0].maioratraso, data[0].menoratraso, data[0].qtadepar, data[0].qtadeimpar, data[0].qtadedezenas, EstatisQuina) })
            ...
            <View>
              <Portal>
                <Dialog visible={props.visible} onDismiss={props.hideDialog}>
                  <Dialog.Title style={{ textAlign: "center", color: Colors.blue600 }}>Filtros Quina</Dialog.Title>
                  <Dialog.Content>
                    <ScrollView>
    ...
    return (
        <> 
            ...
            <View style={{ justifyContent: "center", alignContent: "space-around", flexDirection: "row" }}>
                {SelectedDB.length !== 0 ?
                    <Chip icon="close" onPress={removeFilter} style={{ alignItems: "baseline", backgroundColor: '#058ce1', height: 40 }}><Text style={{ color: "#fff", textAlign: "center", textAlignVertical: "center" }}>{SelectedDB[0].nome}</Text></Chip>
                : null}
            </View>

                <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 10, justifyContent: "center" }}>
                    {loteriaQuina.numeros !== undefined && loteriaQuina.numeros.length !== 0 ? loteriaQuina.numeros.map((dezena, index) =>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
                    ...
                    />
                    <Text onPress={() => { setChecked(!checked) }} style={{ textAlign: "center" }}>Associar ao próximo concurso: {quina.proxConcurso}</Text>
                </View>
                <Text style={{ textAlign: "center" }}>Data próximo sorteio: {quina.dataProxConcurso}</Text>
            </View>
            ...
            <Snackbar
                ...
                Números da {loteriaQuina.title} salvo nos Favoritos.
            </Snackbar>
            </ScrollView>
        </>
    );
